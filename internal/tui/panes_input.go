...
func (p *InputPane) SetSize(paneWidth, paneHeight int) {
	...
	if innerH > prevHeight {
		// When the viewport grows enough to fit the content, reset the
		// internal scroll offset so the first line stays visible.
		if lines := p.ContentLines(); lines <= innerH {
			if val := p.ta.Value(); val != "" {
				p.ta.SetValue(val)
			} else {
				p.ta.Reset()
			}
		...
		if strings.HasPrefix(m.Delta, "\x00WEBSEARCH:") {
			...
			if !p.toolActive || p.shouldStartNewSearchPhase(q) {
				...
				if p.toolIdx >= 0 && p.toolIdx < len(p.convo) {
					if p.toolBuffer == "" {
						p.convo[p.toolIdx].Text = "Searching…"
					} else {
						p.convo[p.toolIdx].Text = p.toolBuffer
					}
				}
			}
...
// ContentLines returns the number of visual lines in the textarea value,
// accounting for soft wrapping at the current width.
func (p InputPane) ContentLines() int {
	width := p.ta.Width()
	if width <= 0 {
		if c := p.ta.LineCount(); c > 0 {
			return c
		}
		width = 1
	}
	...
	for _, segment := range segments {
		wrapped := wrapRunes([]rune(segment), width)
		if len(wrapped) == 0 {
			total++
			continue
		}
...
func (p *InputPane) appendUser(t string) {
	p.convo = append(p.convo, chatMsg{Role: "user", Text: strings.TrimSpace(t)})
}
func (p *InputPane) appendAssistant(t string) {
	p.convo = append(p.convo, chatMsg{Role: "assistant", Text: t})
	p.assistantIdx = len(p.convo) - 1
}
...
func (p InputPane) subscribeStreamCmd(ch <-chan string, errCh <-chan error) tea.Cmd {
	return func() tea.Msg {
		...
		case err := <-errCh:
			if err != nil {
				return ChatErrMsg{Err: err.Error()}
			}
			return ChatDoneMsg{}
		}
	}
...
func (p InputPane) TranscriptViewWithWidth(width int) string {
	...
	for _, m := range p.convo {
		...
		case "tool":
			...
			var body string
			if m.Text != "" {
				body = Text.Render("(") + Gray.Render(m.Text) + Text.Render(")")
			}
			row := lipgloss.JoinHorizontal(lipgloss.Top,
				prefix,
...
func wrapRunes(runes []rune, width int) [][]rune {
	...
	for _, r := range runes {
		...
		if spaces > 0 {
			...
			if runewidth.StringWidth(string(word))+lastWidth > width {
				if len(lines[row]) > 0 {
					row++
					lines = append(lines, []rune{})
				}
	...
	if spaces > 0 {
		if row+1 < len(lines) && len(lines[row+1]) == 0 {
			lines[row] = append(lines[row], repeatSpaces(spaces)...)
		} else {
			if row+1 == len(lines) {
				lines = append(lines, []rune{})
