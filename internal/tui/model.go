...
func (m RootModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
    ...
    case tea.WindowSizeMsg:
        ...
        m.recalcLayout()
        m.vp.Width = m.width
        if m.vp.Width < 1 { m.vp.Width = 1 }
        m.vp.Height = m.height
        if m.vp.Height < 1 { m.vp.Height = 1 }
...
func (m *RootModel) subscribeCmd() tea.Cmd {
    ...
    // store cancel once
    if m.cancelSub == nil { m.cancelSub = cancel }
    return func() tea.Msg {
        e, ok := <-ch
        if !ok { return nil }
        return EventMsg{E: e}
...
func (m *RootModel) recalcLayout() {
    ...
    if inContent < 1 { inContent = 1 }
    notesContent := m.notes.ContentLines()
    if notesContent < 10 { notesContent = 10 }

    inH := inContent + frame
    notesH := notesContent + frame

    paneW := m.width - 2
    if paneW < 10 { paneW = m.width - 1 }
